...
exports.citizenResolvers = {
    ...
    newComplaint: async (args, req) => {
        if (!req.isAuth) {
            ...
            await CITIZEN_MODEL.findOneAndUpdate({ email: req.email }, { $push: { complaints: saved._id } });
            return saved.toString();
        }
    },
    myComplaints: async (args, req) => {
        if (!req.isAuth) {
            throw new Error("Authentication Error");
        }
        else {
    ...
    newReport: async (args, req) => {
        if (!req.isAuth) {
            ...
            const saved = await complaint.save();
            await CITIZEN_MODEL.findOneAndUpdate({ email: req.email }, { $push: { reports: saved._id } });
            return saved.toString();
        }
    },
    myReports: async (args, req) => {
        if (!req.isAuth) {
            throw new Error("Authentication Error");
        }
        else {
